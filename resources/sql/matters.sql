with file_counts as (
    ...
    select matter_id, json_agg(jsonb_build_object('client', row_to_json(c)::jsonb ||
        CASE
...
FROM (
select m.*, COALESCE(fc."files_count", 0) as files_count, c.clients as matter_clients
from "matters" m

left outer join file_counts fc on fc.matter_id = m.id
left outer join matter_clients c on c.matter_id = m.id
